...
fun AstralTopBar(title: String, actions: @Composable RowScope.() -> Unit = {}) {
    val t = rememberInfiniteTransition(label = "g")
    val a by t.animateFloat(0.4f, 1f, infiniteRepeatable(tween(2000), RepeatMode.Reverse), label = "a")
    Column {
        Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 12.dp), verticalAlignment = Alignment.CenterVertically) {
            Box(modifier = Modifier.size(32.dp).clip(CircleShape).background(AL.GoldBg), contentAlignment = Alignment.Center) {
        ...
        HorizontalDivider(color = AL.Border, thickness = 0.5.dp)
    }
}

@Composable
fun AstralBottomBar(selected: Int, onSelect: (Int) -> Unit, badgeCount: Int = 0) {
    NavigationBar(containerColor = AL.Surface, tonalElevation = 0.dp) {
        navTabs.forEachIndexed { i, tab ->
            NavigationBarItem(selected = selected == i, onClick = { onSelect(i) },
                icon = {
                    BadgedBox(badge = {
                        if (i == 0 && badgeCount > 0) Badge(containerColor = AL.Gold) { Text("$badgeCount", fontSize = 8.sp, color = Color.Black) }
                    }) { Icon(tab.icon, tab.label, modifier = Modifier.size(22.dp)) }
                },
                label = { Text(tab.label, fontSize = 10.sp, fontWeight = if (selected == i) FontWeight.Bold else FontWeight.Normal) },
                colors = NavigationBarItemDefaults.colors(
                    selectedIconColor = AL.Gold, selectedTextColor = AL.Gold,
                    unselectedIconColor = AL.Muted, unselectedTextColor = AL.Muted, indicatorColor = AL.GoldBg
                )
            )
        }
    }
...
fun GoldButton(text: String, onClick: () -> Unit, modifier: Modifier = Modifier, icon: ImageVector? = null, enabled: Boolean = true) {
    Button(onClick = onClick, enabled = enabled, modifier = modifier.height(48.dp), shape = RoundedCornerShape(14.dp),
        colors = ButtonDefaults.buttonColors(containerColor = AL.Gold, contentColor = Color.Black,
            disabledContainerColor = AL.GoldDark.copy(0.3f), disabledContentColor = AL.Muted)) {
...
@Composable
fun AstralCard(modifier: Modifier = Modifier, onClick: (() -> Unit)? = null, content: @Composable ColumnScope.() -> Unit) {
    val m = if (onClick != null) modifier.clickable { onClick() } else modifier
    Card(m, shape = RoundedCornerShape(16.dp), colors = CardDefaults.cardColors(containerColor = AL.BgCard), border = BorderStroke(0.5.dp, AL.Border)) {
...
fun StatusChip(text: String, color: Color) {
    Box(modifier = Modifier.clip(RoundedCornerShape(6.dp)).background(color.copy(0.15f)).padding(horizontal = 8.dp, vertical = 3.dp)) {
...
fun GoldDivider() {
    Box(modifier = Modifier.fillMaxWidth().height(1.dp).background(
        Brush.horizontalGradient(listOf(Color.Transparent, AL.Gold.copy(0.3f), Color.Transparent))))
}
...
fun EmptyState(icon: ImageVector, title: String, subtitle: String, action: @Composable (() -> Unit)? = null) {
    Column(modifier = Modifier.fillMaxWidth().padding(vertical = 56.dp), horizontalAlignment = Alignment.CenterHorizontally) {
        Box(modifier = Modifier.size(76.dp).clip(RoundedCornerShape(20.dp)).background(AL.GoldBg), contentAlignment = Alignment.Center) {
            Icon(icon, null, modifier = Modifier.size(38.dp), tint = AL.Gold.copy(0.7f))
        }
        Spacer(Modifier.height(18.dp))
        Text(title, color = AL.White, fontWeight = FontWeight.Bold, fontSize = 18.sp)
        Spacer(Modifier.height(8.dp))
        Text(subtitle, color = AL.Muted, fontSize = 14.sp, textAlign = androidx.compose.ui.text.style.TextAlign.Center, modifier = Modifier.padding(horizontal = 32.dp))
