...
fun EditorTab(code: TextFieldValue, onChange: (TextFieldValue) -> Unit, onRun: () -> Unit, onClear: () -> Unit, onPaste: () -> Unit, output: String?) {
    Column(Modifier.fillMaxSize()) {
        Box(
            Modifier.weight(if (output != null) 0.58f else 0.72f).fillMaxWidth().padding(horizontal = 16.dp)
            ...
            Column(Modifier.fillMaxSize()) {
                ...
                Row(Modifier.fillMaxSize()) {
                    ...
                    Box(Modifier.fillMaxSize().padding(12.dp)) {
                        BasicTextField(
                            value = code, onValueChange = onChange, modifier = Modifier.fillMaxSize(),
        ...
        output?.let { out ->
            Spacer(Modifier.height(10.dp))
            Column(
                Modifier.weight(0.28f).fillMaxWidth().padding(horizontal = 16.dp)
...
fun ConsoleTab(output: String?, onClear: () -> Unit) {
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        ...
        Box(
            Modifier.fillMaxSize().clip(RoundedCornerShape(14.dp)).background(Color(0xFF0D1117))
                .border(BorderStroke(1.dp, AL.Border), RoundedCornerShape(14.dp)).padding(14.dp)
        ) {
            if (output.isNullOrEmpty()) {
                Column(Modifier.fillMaxSize(), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
                    Icon(Icons.Outlined.BugReport, null, tint = AL.Muted.copy(0.3f), modifier = Modifier.size(48.dp))
...
fun DllDropTab() {
    Column(Modifier.fillMaxSize().padding(16.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
        Box(
            ...
            Icon(Icons.Outlined.ArrowUpward, null, tint = AL.Gold, modifier = Modifier.size(40.dp))
        }
        Spacer(Modifier.height(20.dp))
        Text("DLL Injector", color = AL.White, fontWeight = FontWeight.ExtraBold, fontSize = 20.sp)
        Spacer(Modifier.height(10.dp))
        Text(
            "Select a .dll (BepInEx plugin) to add it to the mod list. It will be injected into Among Us when you Patch from the Home screen.",
            color = AL.Muted, fontSize = 14.sp, textAlign = androidx.compose.ui.text.style.TextAlign.Center,
