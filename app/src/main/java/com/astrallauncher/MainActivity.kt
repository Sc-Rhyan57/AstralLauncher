...
fun AppTheme(content: @Composable () -> Unit) {
    MaterialTheme(colorScheme = darkColorScheme(primary = AL.Gold, background = AL.Bg, surface = AL.Surface, onPrimary = Color.Black, onBackground = AL.White, onSurface = AL.White), content = content)
}
...
fun AuMissingDialog(onInstall: () -> Unit, onExit: () -> Unit) {
    Box(Modifier.fillMaxSize().background(AL.Bg), contentAlignment = Alignment.Center) {
        AlertDialog(onDismissRequest = {}, containerColor = AL.Surface, shape = RoundedCornerShape(20.dp),
            title = { Text("Among Us Not Found", color = AL.White, fontWeight = FontWeight.ExtraBold) },
            text = { Text("Install Among Us from the Play Store to use Astral Launcher.", color = AL.Muted, fontSize = 14.sp) },
            confirmButton = { Button(onClick = onInstall, colors = ButtonDefaults.buttonColors(containerColor = AL.Gold), shape = RoundedCornerShape(10.dp)) { Text("Install", color = Color.Black, fontWeight = FontWeight.Bold) } },
            dismissButton = { TextButton(onClick = onExit) { Text("Exit", color = AL.Muted) } })
    }
}
...
fun AstralApp(vm: MainViewModel) {
    ...
    Scaffold(containerColor = AL.Bg,
        topBar = { if (tab !in listOf(1, 2)) AstralTopBar(navTabs[tab].label, actions = { if (tab == 0) IconButton(onClick = { vm.checkGame() }) { Icon(Icons.Outlined.Refresh, null, tint = AL.Muted) } }) },
...
fun CrashScreen(trace: String) {
    ...
    Column(Modifier.fillMaxSize().background(AL.Bg)) {
        Row(Modifier.fillMaxWidth().padding(16.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            TextButton(onClick = { android.os.Process.killProcess(android.os.Process.myPid()) }) { Text("Close", color = AL.Error, fontWeight = FontWeight.Bold) }
        }
        Column(Modifier.fillMaxSize().padding(horizontal = 16.dp).padding(bottom = 16.dp)) {
            Text("An unexpected error occurred. Please report this.", color = AL.Muted, modifier = Modifier.padding(bottom = 12.dp))
            Button(onClick = { val cm = ctx.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager; cm.setPrimaryClip(ClipData.newPlainText("Crash", trace)) },
                modifier = Modifier.fillMaxWidth().padding(bottom = 12.dp), colors = ButtonDefaults.buttonColors(containerColor = AL.Gold), shape = RoundedCornerShape(12.dp)) {
                Icon(Icons.Outlined.ContentCopy, null, modifier = Modifier.size(16.dp), tint = Color.Black); Spacer(Modifier.width(8.dp)); Text("Copy Log", fontWeight = FontWeight.Bold, color = Color.Black)
            }
            Card(Modifier.fillMaxSize(), colors = CardDefaults.cardColors(containerColor = AL.Error.copy(0.08f)), shape = RoundedCornerShape(12.dp)) {
                LazyColumn(Modifier.fillMaxSize()) { item { Text(trace, Modifier.padding(14.dp), fontFamily = FontFamily.Monospace, fontSize = 11.sp, color = AL.Error.copy(0.9f), lineHeight = 17.sp) } }
            }
        }
    }
}
